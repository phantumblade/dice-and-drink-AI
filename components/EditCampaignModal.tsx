...
import { X, Scroll, Users, Calendar, Image, FileText, Sparkles } from 'lucide-react';
...
const EditCampaignModal: React.FC<EditCampaignModalProps> = ({ campaign, onClose, onUpdated }) => {
    ...
    useEffect(() => {
        if (campaign) {
            let parsedTags = '';
            try {
                parsedTags = JSON.parse(campaign.tags || '[]').join(', ');
            } catch (e) {
                parsedTags = campaign.tags || '';
            }
            ...
            setFormData({
                ...
                platform: campaign.platform || 'In Person',
                sessionDuration: campaign.sessionDuration || '3-4 hours',
                tags: parsedTags
            });
        }
    }, [campaign]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await api.put(`/campaigns/${campaign.id}`, updateData);
            showToast('Campagna aggiornata con successo!', 'success');
            onUpdated();
            onClose();
        } catch (error) {
            console.error('Error updating campaign:', error);
            showToast('Errore durante l\'aggiornamento della campagna', 'error');
        }
    };
            ...
            <div className="sticky top-0 bg-white border-b-4 border-black z-10 px-6 py-4 flex justify-between items-center shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="bg-purple-100 p-2 border-2 border-black shadow-neo-sm">
                        <Scroll className="w-6 h-6 text-black" />
                    </div>
                    <div>
                        <h2 className="text-2xl font-black uppercase italic">Modifica Campagna</h2>
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Aggiorna i dettagli dell'avventura</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    <button
                        ...
                        className="px-8 py-2 bg-purple-500 text-white border-2 border-black font-black uppercase shadow-neo hover:translate-y-1 hover:shadow-none transition-all flex items-center gap-2"
                    >
                        <Sparkles className="w-4 h-4" /> Salva Modifiche
                    </button>
                </div>
            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full border-2 border-black p-3 font-bold focus:outline-none focus:ring-4 focus:ring-purple-200 transition-all bg-yellow-50"
                                    >
                                        <option value="RECRUITING">Reclutamento</option>
