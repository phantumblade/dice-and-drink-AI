...
const DnDTracker: React.FC<DnDTrackerProps> = ({ user }) => {
  ...
  const isUserRegisteredForCampaign = (campaign: Campaign) => {
      // Logic: Campaign linked to tournament ID, check if user is in that tournament
      if (!campaign.tournamentId) return true; // Open campaign
      return user.registeredTournaments.includes(campaign.tournamentId);
  };
  ...
  const handleAddCharacter = (e: React.FormEvent) => {
    ...
    const character: Character = {
        ...
        player: newCharacter.player || 'Unassigned'
    };

    const updatedCampaign = {
        ...selectedCampaign,
        party: [...selectedCampaign.party, character]
    };
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto flex items-center justify-between pt-4 border-t-2 border-dashed border-gray-300">
                            <div className="flex -space-x-2">
                                {campaign.party.slice(0, 3).map(char => (
                                    <img key={char.id} src={char.avatar} alt={char.name} className="w-8 h-8 rounded-full border-2 border-white bg-gray-200" />
                                ))}
                                {campaign.party.length > 3 && (
            ...
            <div className="h-48 md:h-64 relative overflow-hidden border-b-2 border-black">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-8">
                    <div>
                        ...
                        <p className="text-neo-yellow font-bold text-lg">{selectedCampaign.system} • DM: {selectedCampaign.dm}</p>
                    </div>
                </div>
                {isLocked && (
            ...
            <div className={`p-8 min-h-[400px] ${isLocked ? 'blur-sm pointer-events-none select-none' : ''}`}>
                ...
                {activeTab === 'party' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {selectedCampaign.party.map(char => (
                            <div key={char.id} className="relative bg-white border-2 border-black p-6 shadow-neo-sm hover:translate-x-1 transition-transform">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t-2 border-gray-100">
                                    <div className="text-center bg-neo-bg p-2 border border-black">
                                        ...
                                        <span className="font-bold">{char.player}</span>
                                    </div>
                                    <button className="bg-black text-white text-xs font-bold uppercase hover:bg-neo-violet transition-colors">
                                        Scheda
                                    </button>
                                </div>
                            </div>
